...
'''
(1) X16 = L0 (9 bits) || R0 (7 bits), KIij = KIijL || KIijR
(2) R1 = S9(L0) ⊕ Extnd(R0)
(3) L1 =R0
(4) R2 = S7(L1) ⊕ Trunc(R1) ⊕ KIijL
(5) L2 = R1 ⊕ KIijR
(6) R3 = S9(L2) ⊕ Extnd(R2)
(7) L3 = R2
(8) Y16 = L3 || R3
...
"""
(1) X32 = L0 || R0
KOi= KOi1 || KOi2 || KOi3 || KOi4, KIi= KIi1 || KIi2 || KIi3
...
(3) Y32 = (L3⊕ KOi4) || R3
"""
def FO(value: int, k: int, key_data: dict) -> int:
    KO = key_data["KO"][k]
    KI = key_data["KI"][k]
    ...
    for j in range(3):
        ...
        Rj_1 = R[-1]
        Lj = Rj_1
        Rj = FI(Lj_1 ^ KO[j], KI[j]) ^ Rj_1
        L.append(Lj)
        R.append(Rj)

    L3 = L[-1] ^ KO[3]
    R3 = R[-1]
    return ((L3 << 16) | R3) & 0xFFFFFFFF

'''
(1) X32 = XL || XR, KLi= KLiL || KLiR
(2) YR = (XL ∧ KLiL) ⊕ XR
(3) YL = XL⊕ (YR ∨ KLiR)
(4) Y32 = YL || YR
'''
def FL(value: int, k: int, key_data: dict) -> int:
    KL_L, KL_R = key_data["KL"][k]
    XL = (value >> 16)
    XR = value & 0xFFFF
    YR = (XL & KL_L)^XR
    YL = XL ^ (YR | KL_R)
...
(4) X32 = XL || XR
'''
def FL_INV(value: int, k: int, key_data: dict) -> int:
    KL_L, KL_R = key_data["KL"][k]
    YL = (value >> 16)
    YR = value & 0xFFFF
    XL = YL^(YR| KL_R)
